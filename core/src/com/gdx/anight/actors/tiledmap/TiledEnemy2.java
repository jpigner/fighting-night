...
public class TiledEnemy2 extends GameActorEnemyTiled{
	...
	public TiledEnemy2(Body body) {
		super(body);
		
		body.setActive(false);
		
		enemy = new Sprite(Assets.manager.get(Textures.TEX_PLAYERSHEET2, Texture.class));
		...
		hurt_bilder = BatchUtils.createFramesZeile(regions_enemy, hurt_bilder, 0, 9, 4);
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		if(ContaktListener.isCollide() && ContaktListener.isEnemy2()) {
				if(GameStage.getEnemies2().get(ContaktListener.getEnemyIndex()).getUserData().getHealth() > 0) {
					BatchUtils.drawBodyLife(batch, empty_life, rect, GameStage.getEnemies2().get(ContaktListener.getEnemyIndex()).getUserData().getBody(),
							(GameStage.getEnemies2().get(ContaktListener.getEnemyIndex()).getUserData().getMaxHealth())/16, 10, 1, 1);
					BatchUtils.drawBodyLife(batch, full_life, rect, GameStage.getEnemies2().get(ContaktListener.getEnemyIndex()).getUserData().getBody(),
							(GameStage.getEnemies2().get(ContaktListener.getEnemyIndex()).getUserData().getHealth())/16, 10, 1, 1);
				}
		}
		
		for (int i = 0; i<GameStage.getEnemies2().size; i++) {
		if(GameStage.getEnemies2().get(i).getUserData().getHealth() <= 0) {
			BatchUtils.drawBodyEnemy(batch, dead, rect, GameStage.getEnemies2().get(i).getUserData().getBody(), 1, 1.3f);
			GameStage.getEnemies2().get(i).getUserData().getBody().setActive(false);
			GameStage.getEnemies2().get(i).getUserData().setCurrentState(EnemyStates.DEATH);
			GameStage.getEnemies2().get(i).getUserData().setDead(true);
		}
		}
	...
	private void handleEnemyAttack() {
		 if(!Player.isAttack() && TiledEnemy2.isFighting() && 
				 (GameStage.getEnemies2().get(ContaktListener.getEnemyIndex()).getUserData().getBody().getPosition().x < 
			 ...
			 GameStage.getPlayer().getUserData().setHealth(GameStage.getPlayer().getUserData().getHealth() -
					 GameStage.getEnemies2().get(ContaktListener.getEnemyIndex()).getUserData().getDamage());
		 }
	}
	...
	public static void setCurrentState(EnemyStates currentState) {
		TiledEnemy2.currentState = currentState;
	}
